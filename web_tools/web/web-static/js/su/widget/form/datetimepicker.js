    ...
    function DateTimePicker(element, options) {
        var picker = {},
            ...
            _createDatePicker = function () {
                ...
                $table.$prevm = _createCell($row, '<span class="date-picker-icon icon-datepicker-prev"/>', 1, NAV['prevm'], "prevm");
                //head - tools - title
                $table.$title = $('<td class="title" colspan="5"/>').data('nav', NAV['title']).appendTo($row);
                //head - tools - prev month
                $table.$nextm = _createCell($row, '<span class="date-picker-icon icon-datepicker-next"/>', 1, NAV['nextm'], "nextm");
                $row.appendTo($thead);
                //head - week names
                $row = $('<tr/>');
                var i;
            ...
            _createTimePicker = function (viewmode) {
                ...
                $table.focus = $table.$m;
                var $add = $('<td/>').append($('<span class="date-picker-icon icon-datepicker-plus"/>')).data('nav', NAV['plus']);
                var $min = $('<td/>').append($('<span class="date-picker-icon icon-datepicker-minus"/>')).data('nav', NAV['minus']);
