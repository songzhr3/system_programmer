...
$.su.Widget("portrange", {
	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			inputStartPort.attr("maxlength", 5);
	...
	validate: function(me, flag){
		...
		$startPort.val(valueStartPort);
		$endPort.val(valueEndPort);
/*		if ($.type(tar.textFormat) == "function"){
		...
		if (tarStartPort && tarEndPort){
			...
			if (valueStartPort === "" || valueEndPort === ""){
				...
				if(valueStartPort !== ""){
					result = new $.su.vtype({vtype:'number'}).validate(valueStartPort);
					if (result !== true){
						return returnResult("错误的格式，仅允许输入数字");
					}
				}
				if(valueEndPort !== ""){
					result = new $.su.vtype({vtype:'number'}).validate(valueEndPort);
					if (result !== true){
						return returnResult("错误的格式，仅允许输入数字");
					}
				}
	...
	setValue: function(me, _value){
		...
		if (value === "" || value === undefined || value === null || value.indexOf("-") == -1){
			...
			var pattern = /^(-\d+?|\d*)-(-\d+?|\d*)$/;
            var portrange = pattern.exec(value);
			if(portrange == undefined || portrange.length < 3){
				me.val("");
				$endPort.val("");
				$startPort.val("");
			}
			else{
				$startPort.val(portrange[1]);
				$endPort.val(portrange[2]);
				me.val(value);
			}
		};
