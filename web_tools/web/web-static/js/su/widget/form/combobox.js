...
$.su.Widget("combobox", {
	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			if (target.parent("div.combobox-container").length == 0){
				target.replaceWith(container);
				container.prepend(target.addClass("hidden combobox-value").addClass(this.allowChunk?"chunk":"").prop("disabled", true));
				me.combobox("setText");
			}
		});
		...
		container.delegate("a.combobox-switch", "click", function(e){
			...
			me.combobox("setTips", me.get(0).tips);
		...
		container.delegate("input.combobox-text", "click", function(e){
			...
			var chunk_obj = $(this).closest("div.combobox-container").find("input.chunk");
			if ($(this).is(":checked")){
				//$(this).addClass("selected");
				chunk_obj.combobox("enable");
			}else{
				chunk_obj.combobox("disableNoCheckbox");//checkbox需一直生效
				//$(this).removeClass("selected");
			}
			if (chunk_obj.get(0) && chunk_obj.get(0).callback){
				chunk_obj.get(0).callback.call(this, $(this).is(":checked"));
			}
	...
	loadData: function(me, items){
		...
		//减少不必要的find操作，解决数据量过大导致载入缓慢的问题
		var combo_list_obj = container.find("ul.combobox-list");
		for (var index = 0, len = items.length; index < len; index++){
			if (items[index]){
		...
		me.combobox("setText");
		/*
		if (obj.bigdata){//减少不必要的find操作，解决数据量过大导致载入缓慢的问题
	...
	setText: function(me, arguments){
		...
		if (_valueType === "array"){
			for (var index = 0; index < _value.length; index++){
				...
				for (var i = 0; i < optionItems.length; i++) {
					if (optionItems[i].value == _value[index]) {
						_name = $(optionItems[i]).attr("display");
						break;
					}
				}
				text = text + _name + ",\x20";
		...
		if (text == ""){
			textBox.val(me.get(0).noneSelectedTextLocal ? "" : me.get(0).noneSelectedText);
		}else{
			textBox.val(text);
		};
	...
	disableStyle: function(me){	//å¯¹æ•´ä¸ªå¯¹è±¡å®žçŽ°ç¦ç”?
		var me = me || this,
			container = me.combobox("getContainer");
	...
	select: function(me, valueArray){
		...
		if (obj.alwaysTrigger || vOld.toString() != vNew.toString()){
			$(obj).trigger("ev_change", [vOld, vNew]);
		};
	...
	collapse: function(me) {
		var me = me || this;
		var wrap = me.siblings().find("div.combobox-list-wrap");
		wrap.fadeOut(500);
		wrap.attr("toggleflag", "hidden");
		return me;
	}
